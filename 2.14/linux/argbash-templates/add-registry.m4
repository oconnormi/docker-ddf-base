...
# ARG_OPTIONAL_BOOLEAN([auto-push], [a], [Enable Auto-Pushing to the registry])
# ARG_OPTIONAL_SINGLE([name], [n], [Full name of registry, Defaults to the ID when omitted])
# ARG_OPTIONAL_SINGLE([short-name], [s], [Short name of registry. Defaults to the registry ID when omitted])
# ARG_OPTIONAL_SINGLE([id], [i], [Registry ID. Gets autogenerated when omitted])
...
# ARG_DEFAULTS_POS
# ARG_HELP([Adds a remote registry configuration for pushing and pulling source configurations], [This utility treats the registry id as the unique identifier. If the command is re-run with the same id it will update an existing registry configuration with that id. All registry interaction is turned off unless options are enabled for push, pull, etc. If the 'id' option is not specified it will be set to the value of the url. All other identifiers such as name and shortname will default to same value as 'id' when not specified. Templates: All configurations are generated from '.template' files located in the registry templates directory. Template Directory: ${_registry_template_directory}. To set an alternate template directory override the 'REGISTRY_TEMPLATE_DIRECTORY' variable.])
# ARGBASH_GO
...
function configExists {
  shopt -s extglob
  local result=$(find ${_arg_config_directory} -type f -name "${_service_name}*" -exec grep -H "${_arg_id}" {} \; | wc -l)
  result=${result##*( )}
  result=${result%%*( )}
  shopt -u extglob
  echo "${result}"
...
function createConfig {
    ...
    unset _id _name _short_name _username _password _url _pid _push _pull _auto_push
    return ${_success}
}
